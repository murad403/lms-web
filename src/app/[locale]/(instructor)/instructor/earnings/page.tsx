...
import { useCancelWithdrawRequestMutation, useEarningsQuery, useStripeConnectMutation, useStripeDashboardLinkMutation, useWithdrawRequestMutation } from "@/redux/features/instructor/instructor.api";
...
const EarningsPage = () => {
    ...
    const chartData = useMemo(
        () =>
            (earningsData?.monthly_revenue_chart ?? []).map((item) => ({
                label: item.label,
    ...
    const historyRows = useMemo<WithdrawHistoryRow[]>(
        () =>
            (earningsData?.withdrawals ?? []).map((item) => ({
                id: item.id,
