            ...
            <div className="px-3 md:px-0 container mx-auto">
                {
                    categories.map((category: TCategory) =>
                        <Link className="mb-5 block" key={category.id} href={"/"}>
                            <h3 className={`text-lg font-bold text-white ${category?.description ? "rounded-t-lg" : "rounded-lg"} p-5 ${category.headingColor}`}>{category.title}</h3>
                            <p className={`${category?.description ? "p-5" : "p-0"} rounded-b-lg ${category.descriptionColor}`}>{category?.description}</p>
                        </Link>
                    )
                }
            </div>
