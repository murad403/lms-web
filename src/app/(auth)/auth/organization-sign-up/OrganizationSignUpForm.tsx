...
import { organizationSignUpSchema, type OrganizationSignUpFormData } from '@/validation/auth.validation';
...
const OrganizationSignUpForm = () => {
  ...
  const onSubmit = async (data: OrganizationSignUpFormData) => {
    ...
    console.log('Organization sign up:', data);
  };
  ...
  return (
    <div className="w-full lg:w-1/2 flex flex-col justify-center px-6 sm:px-12 lg:px-20 py-10 overflow-y-auto">
      ...
      <Link href="/" className="flex items-center gap-2 mb-6">
        ...
        <span className="text-2xl font-bold text-main">Form-Cert</span>
      </Link>

      <h1 className="text-3xl font-bold text-header mb-2">Organization Registration</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-md">
        ...
        <label className="flex items-start gap-2 cursor-pointer">
          ...
          <span className="text-sm text-description">
            I agree to the{' '}
            <Link href="#" className="text-main hover:underline">Terms of Service</Link>
            <span> and </span>
            <Link href="#" className="text-main hover:underline">Privacy Policy</Link>
          </span>
        </label>
        ...
        <button
          ...
          className="w-full py-3 bg-main text-white font-semibold rounded-lg hover:bg-main/90 transition disabled:opacity-50 cursor-pointer"
        >
          Create Account
        </button>
      </form>

      <p className="text-sm text-description mt-6">
