...
type Props = {
  ...
  setUploadedVideo: (
    val: { name: string; preview: string; file?: File } | null
  ) => void;
...
const LectureVideoModal = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-[calc(100vw-32px)] sm:max-w-lg">
        <DialogHeader>
        ...
        <div className="space-y-4 mt-2">
          {uploadedVideo ? (
            <div className="flex gap-4 items-start min-w-0">
              <div className="w-32 h-20 bg-gray-100 rounded-md overflow-hidden shrink-0">
                <video
                  src={resolveImageUrl(uploadedVideo.preview)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p
                  ...
                  className="text-sm text-title break-all leading-relaxed"
                >
                  {uploadedVideo.name}
                </p>
          ...
          <div className="flex justify-end gap-3">
            <button
              ...
              onClick={onClose}
              disabled={isSaving}
              className="px-4 py-2 border border-border-light rounded-md text-sm font-medium text-title hover:bg-gray-50 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {t("cancel")}
            </button>
