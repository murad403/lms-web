...
import { QuizItem } from "@/redux/features/create-course/createCourse.type";
...
// Convert API QuizItem → local QuizData
export const quizItemToQuizData = (quiz: QuizItem): QuizData => ({
    title: quiz.title,
    description: quiz.description,
    timeLimit: String(quiz.time_limit_minutes),
    attemptsAllowed: String(quiz.attempts_allowed),
    passingScore: String(quiz.passing_score),
    ...
    questions: quiz.questions.map((q) => ({
        ...
        options: q.options.map((opt, idx) => ({
            label: q.question_type === "mcq"
                ...
                : opt.text === "true" || opt.text.toLowerCase() === "true" ? "True" : "False",
            value: opt.text,
    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="w-[calc(100%-2rem)] max-w-full xl:max-w-285 max-h-[90vh] overflow-y-auto lg:left-[calc(50%+10rem)] lg:w-[calc(100%-20rem-2rem)] lg:max-w-285">
                <DialogHeader>
                    <DialogTitle className="text-xl font-semibold text-main">Edit Quiz</DialogTitle>
                                            ...
                                            {question.options.map((option, oIndex) => (
                                                <div key={oIndex} className="flex items-center gap-2">
                                                    <button
                                                        ...
                                                        className={`size-9 flex items-center justify-center rounded-md text-sm font-medium transition-colors shrink-0 ${
                                                            option.isCorrect
                                                                ? "bg-main text-white"
...
export { type QuizData, type QuizQuestionType, type QuizOption, type QuizQuestion };
